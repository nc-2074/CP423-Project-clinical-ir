...
def pipeline():
    ...
    print(f"Indexing transcript with session_id: {session_id}")
    index_transcript(
        transcript,
        supabase=supabase,
...
# ── Generate real LiveKit JWT tokens ───────────────────────────────────
@app.route("/live/tokens", methods=["POST"])
def live_tokens():
    """
    Generate real LiveKit JWT tokens for patient and clinician.
    Replaces the mock tokens that were previously hardcoded in main.js.
 
    Request body (optional):
        { "room_name": "my-room" }   ← defaults to auto-generated name
...
# ── Get live transcript (polling) ──────────────────────────────────────
@app.route("/live/transcript", methods=["GET"])
def live_transcript():
    """
    Return current transcript segments for an active live session.
    The frontend polls this every few seconds to show live updates.
...
def live_stop():
    ...
    # Index into Supabase
    session_id = str(uuid.uuid4())
    print(f"Indexing live transcript — session_id: {session_id}")
    index_transcript(
        segments,
        supabase=supabase,
        embedding_model=embedding_model,
